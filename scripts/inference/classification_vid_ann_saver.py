...
#! Video Writer Configurations
RAW_VIDEO_PATH = "sample_videos_output/raw_video/09302025.mp4"
METADATA_PATH = "sample_videos_output/raw_video/09302025_metadata.jsonl"  # JSON Lines format for streaming appends
...
def compute_smoothed_classes(
    tracks: np.ndarray, class_history: Dict[int, Deque[np.ndarray]]
    ...
    for track in tracks:
        ...
        smoothed.append((x1, y1, x2, y2, track_id, score, smoothed_class_id))

    return smoothed


def draw_visualizations(frame, smoothed_tracks, track_history):
    if len(smoothed_tracks) == 0:
        return frame

    for x1, y1, x2, y2, track_id, score, smoothed_class_id in smoothed_tracks:
...
class VideoThread(QThread):
    ...
    def writer_loop(self):
        ...
        with open(METADATA_PATH, "w") as metadata_file:
            while self.writer_running or not self.write_queue.empty():
                try:
                    raw_frame, meta = self.write_queue.get(timeout=1)
                    if video_writer is None:
                        h, w = raw_frame.shape[:2]
                        video_writer = cv2.VideoWriter(
                            RAW_VIDEO_PATH, fourcc, VIDEO_FRAME_RATE, (w, h)
                        )
                    video_writer.write(raw_frame)
                    json.dump(meta, metadata_file)
                    metadata_file.write("\n")
                    metadata_file.flush()  # Ensure data is written
                except queue.Empty:
                    continue
        if video_writer:
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Compute smoothed classes
            smoothed_tracks = compute_smoothed_classes(
                display_tracks, self.class_history
            )
                ...
                "detections": [
                    {
                        ...
                        "class_id": int(smoothed_class_id),
                        "class_name": CLASS_NAMES[smoothed_class_id],
                        "score": float(score),
                    }
                    for x1, y1, x2, y2, track_id, score, smoothed_class_id in smoothed_tracks
                ],
            ...
            detected_frame = draw_visualizations(
                frame, smoothed_tracks, self.track_history
            )
            ...
            # Send raw frame and metadata to writer queue
            self.write_queue.put((frame.copy(), metadata))
