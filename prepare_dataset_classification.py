...
def prepare_data(dataset_root='dataset_classification', output_dir='coco_classification', train_split=0.8, seed=42):
    ...
    negative_dir = os.path.join(dataset_root, 'negative_samples') if ADD_NEGATIVE_SAMPLES else None
    ...
    mask_files = [f for f in os.listdir(masks_dir) if f.lower().endswith(mask_extensions)]
    ...
    print(f"Found {len(positive_basenames)} positive image-mask pairs out of {len(image_basenames)} images and {len(mask_basenames)} masks.")

    # Negative samples if enabled
    negative_basenames = []
    if ADD_NEGATIVE_SAMPLES and os.path.exists(negative_dir):
        negative_files = [f for f in os.listdir(negative_dir) if f.lower().endswith(img_extensions)]
    ...
    # Process each split
    for split, split_basenames, folder in [('train', train_basenames, 'train2017'), ('val', val_basenames, 'val2017')]:
        images = []
        annotations = []
        ann_id = 1
        ...
        for image_id, basename in enumerate(split_basenames, start=1):
            ...
            img_file_candidates = [f for f in os.listdir(source_dir) if f.rsplit('.', 1)[0] == basename and f.lower().endswith(img_extensions)]
            if not img_file_candidates:
                print(f"Warning: No image file found for basename {basename} in {source_dir}")
                continue
            ...
            if is_positive:
                ...
                mask_file_candidates = [f for f in mask_files if f.rsplit('.', 1)[0] == basename]
